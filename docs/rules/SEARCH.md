...
6. **順位**：（一致した断片の種類数↓、総出現数↓、直接一致〔対応表を経由しない〕した断片の数↓、論文順 T1→T5↑、行番号↑）の全順序（SPEC v2.1 §5.3 の順。段階0の付録B-2 では直接一致を総出現数より先に置いていたが、正本に合わせた）。スコアとしては rank と rank_key を返し、確率にはしない。
