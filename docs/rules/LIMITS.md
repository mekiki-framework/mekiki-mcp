...
| 項目 | 値 |
|---|---|
| 規則ID・版 | LIMITS-3.1.0（2026-09-19・配置段階二。配置モード spaces の値〔待ち受け・許可 Host・`/` の Host 不問〕を項目として足した。local の値は変えていない。同日 Codex④ で、spaces のポートを SPEC v2.4.2 の 7860 固定に合わせ（F7）、Host のポートの桁数（5桁以内）を明記した（F4）〔3.1.0 は提案のまま、正本 v2.4.2 が 3.1.0 を指すので版は据え置き〕。同日、`/gradio_api/mcp` の 307 をやめて同じ本体として扱い、転送を一切返さないことを開放経路の行に追記した〔応答の `limitations` に載る版と正本の参照を変えないため、F7 と同じく版は据え置き〕）。LIMITS-3.0.0（2026-09-19・Codex③。受付枠を本文の前へ移し、種類ごとの受付枠・送信期限・起動の準備を足し、回収されない結果の数え方〔できた時刻・UTF-8 の JSON〕を変えた＝既存の値の意味を変えたので MAJOR。ログの項目を追加）。LIMITS-2.0.0（2026-09-19。「開放経路」の値を変えた＝旧 SSE の `/gradio_api/mcp/sse`・`/gradio_api/mcp/messages/` と Streamable HTTP の別名 `/gradio_api/mcp/http` を閉じたので MAJOR。あわせて「長時間接続」の項目を足した）。1.1.0（2026-09-18 著者承認）は 1.0.0 の値を変えずに、関連原文の抜粋・check_compressions の結果数・通信層・開放経路の項目を足した版 |
...
| Host ヘッダ | local：127.0.0.1・localhost・::1。spaces（3.1.0）：起動時に読んだ `SPACE_HOST`（カンマ区切りは各値・小文字にそろえる）と localhost・127.0.0.1（::1 は含めない。`SPACE_HOST` が無いか形が違えば起動しない）。どちらも欠落・重複は不可。ポートは省略か、5桁以内の十進で1〜65535のときだけ認める（先に桁数を見る。`065535` のような6桁は不可。Codex④ F4） | HTTP 400。spaces の `/` の GET・HEAD だけは Host を問わず 200（健康検査のため）。許可していない Host には Gradio の画面ではなく固定の短い HTML を返す |
...
| 回収されない結果（3.0.0 で数え方を変更） | **結果ができた時刻から** 120 秒、同じセッションで 64 件・4 MiB（**`/queue/data` が送る UTF-8 の JSON で数える**）。5秒ごとに掃除し、古いものから捨てる。取りに来ている（`/queue/data` が開いている）セッションは期限だけを免除し、件数・大きさの上限は当てる。内部クライアントのセッションには手を付けない。セッションそのものは消さない（処理中の呼び出しが結果を入れる先を失わない）。セッションの表は2,000件（上流の LRU）で、追い出すときにそのセッションのイベント ID・イベント・状態・時刻の記録を一緒に消す。内部クライアント・回収中・処理中のセッションは追い出さない | 期限・件数・大きさを超えた結果を捨てる |
| アップロードの大きさ | `max_file_size="1kb"`（経路自体は 403 で遮断） | HTTP 403 |
| 開放経路 | `/`（起動時の到達確認と、`/config` を塞いだときに自己呼び出しが設定を読む先）・`/gradio_api/startup-events`（起動時の確認）・`/gradio_api/info`（末尾 `/` 付きも）・`/gradio_api/queue/join`・`/gradio_api/queue/data`・`/gradio_api/heartbeat/*`（自己呼び出し）・`/gradio_api/mcp/`（MCP 本体。`/gradio_api/mcp`〔末尾 `/` なし〕も完全一致で許可し、転送せずサーバの中で `/` 付きと同じ本体として扱う〔3.1.0 の追記・2026-09-19〕）・`/gradio_api/mcp/schema`（ツールの JSON スキーマ）。`/config` は塞いでも動き再試行も起きないので塞いだ。heartbeat は内部クライアントが使い、上流のクライアントが断られると間を置かずに再試行するので、塞がずに同時数で絞る（上流との互換）。**2.0.0 で `/gradio_api/mcp/sse`・`/gradio_api/mcp/messages/`・`/gradio_api/mcp/http`（旧 SSE の予備経路と、Streamable HTTP の別名。/ 付きも）を閉じた**（閉じても M01〜M03・SDK 検収・`mcp-remote@0.14.2 --transport http-only` が動くことを実測・2026-09-19）。`/gradio_api/mcp/schema` は三機能には要らないが著者の指示で開けている | ほかは 403（明示の遮断）か 404。**転送（3xx）は一切返さない**（プロキシの裏では上流の `Location` が `http://` になるため。上流が返す転送〔実測では `/gradio_api/heartbeat/<id>/`〕は 404 に置き換え、`Location` を出さない。3.1.0 の追記・2026-09-19） |
