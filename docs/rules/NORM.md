...
| 項目 | 値 |
|---|---|
| 規則ID・版 | NORM-1.2.0（2026-09-19・著者の指示。規則8の適用を空白の左右両方に明記し、規則9〔強調記号だけを挟んだ空白の並びの畳み込みと、先頭・末尾の強調記号の差分〕を足した。Codex③ 10）。1.1.0 は 1.0.0 に MARK-EMPH を足した版（2026-09-18・著者承認） |
| 状態 | 確定（回答票 Q41〜Q49、2026-09-18） |
| 実装 | 施工段階2で実装（`mekiki_reader/normalize.py`）。表の正準 JSON の SHA-256：`19f90a79f945e17050264999caf4cd06dbe1056af0272ebfe079a9e10b16f6f0`（`NORM_TABLE_SHA256`。import 時に再計算して照合する。Q49） |
...
8. 強調記号（MARK-EMPH）は空白の文脈判定でも「意味のある文字」として数えない（`**参加** の` のような並びで、記号が空白の扱いを変えないように）。空白の左側・右側のどちらに記号があっても同じ（1.2.0 で右側も明記）。
9. 空白の並びが強調記号（とゼロ幅文字）だけを挟んで続くときは、一つの空白に畳む（WS-COLLAPSE。並びにゼロ幅文字があれば WS-ZW も記録する）。先頭・末尾で落とした強調記号も payload.diffs に MARK-EMPH として残す（空白だけの前後は、これまでどおり normalization_applied にだけ出る）。（1.2.0）
