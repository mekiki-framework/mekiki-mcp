...
3. 断片が一つ以上一致した行を、SEARCH-1.0.0 の順位（語の種類数↓→総出現数↓→直接一致↓→論文順↑→行番号↑）で並べ、上位5件を candidates に出す。payload に `match: "none"`・`partial`・抜粋・一致位置を付ける。
...
3. 一つの単位に全断片が一致した主張を results（全語一致の多い順→総出現数→直接一致→台帳の順）、一部だけ一致した主張を candidates（上位5件）に出す。payload.match に一致した項目名・断片・経路・順位を付ける。
