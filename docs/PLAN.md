...
- 公開前の作業（Q95）：DECISIONS.md の「正本の配置」行の理由欄にあった非公開資料への参照を「方針書 v2 冒頭の改版行」に置き換える。「段階二」の表記を「配置段階二」に統一する。Q64・Q66⑥ の上流の挙動の分析を要約にする（未報告の上流不具合の詳細は出さない）。→2026-09-19 Codex③ 9 で①〜③とも実施（②は必須記録項目の表記）。
