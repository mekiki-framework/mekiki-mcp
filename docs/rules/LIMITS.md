...
| 要求本文 | 64 KiB。**表明ではなく実際に届いたバイト数**で打ち切る | HTTP 413 |
| 本文の長さの表明 | `Content-Length` は ASCII 数字だけ・重複不可。`Transfer-Encoding` との併記は不可。表明と実測の食い違いも不可 | HTTP 400 |
| Host ヘッダ | 127.0.0.1・localhost・::1（欠落・重複も不可） | HTTP 400 |
| 自己呼び出しの経路（`/gradio_api/queue/join`） | 同時 8（＝同時実行4＋4）。待ち行列は 16 | HTTP 503 |
