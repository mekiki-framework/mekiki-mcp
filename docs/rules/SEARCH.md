...
1. **検索用の正規化**：NORM-1.0.0 の WS-ZW・WS-COLLAPSE・WIDTH（ASCII と半角カナ）・QUOTE-CURLY 規則に、ASCII の A–Z → a–z（規則ID `ASCII-LOWER`）を加える（WS-CJK と PUNCT は使わない）。クエリ（入力）には先に NFC-IN をかける。原文の各行にも同じ規則をかけ、区間の表で原文の位置へ戻す。’ は QUOTE-CURLY で ' になる。実装は、クエリ全体を畳み込んでから2の区切りで分ける（WS-CJK を使わないので、先に分けてから畳み込むのと結果は同じ。全角の区切り文字は WIDTH で ASCII に写ってから判定される）。
