...
`derivative_of` の欄は実装を呼び出して確かめた実測値（2026-09-18・PATTERNS-0.1.1・49件を載せた状態）。
...
| SOURCE_INDEX.md・llms.txt | reading_guide | （PATTERNS-0.1.1 が引いていないので現状は出ない。種別としては reading_guide なので、将来の承認パターンが引けば論文五本で出る） | 出ない |
